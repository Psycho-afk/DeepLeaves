...
# TODO Cargar el archivo JSON con los nombres de los Pokémon
#with open('/Users/Mosk/Documents/Pplantas/plantas.json', 'r') as json_file:
 #   nombres_pokemon = json.load(json_file)
...
def predict_pokemon():
    ...
    if data:
        ...
        pokemon_predicho = predecir_pokemon(temp_filename) # TODO

        # Calcular similitudes y obtener los 5 Pokémon más similares
        similitudes = calcular_similitud(temp_filename)
        indices_similares = np.argsort(similitudes)[::-1][:2] #? Cuatro de las 4 especies sobrantes 
...
class PredictorPokemon(QWidget):
    ...
    def predecir_pokemon(self):
        if hasattr(self, 'ruta_imagen'):
            ...
            indices_similares = np.argsort(similitudes)[::-1][:2]  # Obtener los 5 Pokémon más similares
